...
let personalityInfo = 'No personality data. Skip Task-Trait Alignment, use neutral defaults.';
...
if (hasPersonality) {
  ...
  if (p.trait_percentages) {
    ...
    if (t.energy !== undefined) traits.push(`${mainType[1]} (${t.energy}%)`);
    if (t.nature !== undefined) traits.push(`${mainType[2]} (${t.nature}%)`);
    if (t.tactics !== undefined) traits.push(`${mainType[3]} (${t.tactics}%)`);
    if (t.identity !== undefined) traits.push(`${identity} (${t.identity}%)`);
