...
let personalityDoc = referenceData.personality_calibration_doc;
if (!webhookData.user_context?.personality?.personality_type) {
    personalityDoc = {
        content: 'Not applicable - no personality data provided',
    };
}
...
if (userContext) {
    ...
    if (userContext.location?.country) {
        parts.push(
            `Location: ${userContext.location.country}${userContext.location.city ? ` (${userContext.location.city})` : ''}`,
        );
        if (userContext.location.timezone)
            parts.push(`Timezone: ${userContext.location.timezone}`);
        if (userContext.location.currency)
            parts.push(`Currency: ${userContext.location.currency}`);
        if (userContext.location.language)
            parts.push(`Language: ${userContext.location.language}`);
    }

    // Professional context
    if (
        userContext.professional?.job_title ||
        userContext.professional?.industry
    ) {
        ...
        if (prof.experience_level)
            profParts.push(`(${prof.experience_level} level)`);
    ...
    if (userContext.preferences?.primary_programming_language) {
        parts.push(
            `Primary language: ${userContext.preferences.primary_programming_language}`,
        );
    }
...
// Helper function to extract display value from context field
const getContextValue = (field) => {
    if (!field) return null;
    if (typeof field === 'string') return field; // Direct string value
    return field.answer_label || field.answer || null; // Nested object: prefer label
};

if (
    preAnalysisContext &&
    typeof preAnalysisContext === 'object' &&
    Object.keys(preAnalysisContext).length > 0
) {
    preAnalysisSection =
        '\n\n**Pre-Analysis Context:**\nContext gathered during pre-analysis (either from user answers or inferred):';

    // Format the context
    if (preAnalysisContext.subject) {
        preAnalysisSection += `\n- Subject/Topic: ${preAnalysisContext.subject}`;
    }
    const audienceValue = getContextValue(preAnalysisContext.audience);
    if (audienceValue) {
        ...
        const audienceLabel = audienceLabels[audienceValue] || audienceValue;
        preAnalysisSection += `\n- Audience: ${audienceLabel}`;
    }
    if (preAnalysisContext.purpose) {
        preAnalysisSection += `\n- Purpose: ${preAnalysisContext.purpose}`;
    }
    const detailValue = getContextValue(preAnalysisContext.detail_level);
    if (detailValue) {
        const detailLabels = {
        ...
        const detailLabel = detailLabels[detailValue] || detailValue;
        preAnalysisSection += `\n- Detail Level: ${detailLabel}`;
    }
...
console.log(preAnalysisSection);
