...
class FrameworkAnalyticsService
{
    ...
    public function recordSelection(
        ...
        try {
            $selection = FrameworkAnalytic::create([
    ...
    public function updateWithOutcome(
        ...
        try {
            $selection->update([
                ...
                'rating_explanation' => $ratingExplanation ?? $selection->rating_explanation,
    ...
    {
        $total = FrameworkAnalytic::where('recommended_framework', $framework)->count();

        if ($total === 0) {
            return 0;
        }

        $accepted = FrameworkAnalytic::where('recommended_framework', $framework)
    ...
    {
        return FrameworkAnalytic::where('chosen_framework', $framework)
    ...
    {
        $total = FrameworkAnalytic::where('chosen_framework', $framework)->count();

        if ($total === 0) {
            return 0;
        }

        $copied = FrameworkAnalytic::where('chosen_framework', $framework)
    ...
    public function getFrameworkPerformance(string $framework): array
    {
        return [
            'framework' => $framework,
            'total_recommended' => FrameworkAnalytic::where('recommended_framework', $framework)->count(),
            'total_chosen' => FrameworkAnalytic::where('chosen_framework', $framework)->count(),
    ...
    {
        $frameworks = FrameworkAnalytic::distinct('chosen_framework')
