...
This document provides the AI system with comprehensive information about prompt frameworks, task categories, and the
mapping between them. Use this to classify user tasks and select the most appropriate framework.
...
Each task category has inherent cognitive requirements that may align with or oppose specific personality traits. This
information is used for Task-Trait Alignment analysis (amplification, counterbalancing, or neutral handling).
...
| `ITERATIVE`      | Iterative Self-Improvement — multiple passes, self-critique, progressive refinement                |
| `ABSTRACTION`    | Abstraction Before Specifics — principle identification before tackling concrete details           |
| `AGENTIC`        | Agentic Reasoning — interleaved thinking and acting with external tool interaction                 |
...
| `CREATION_CONTENT`   | Varies by content type (see below) | `STRUCTURE`, `ITERATIVE`     |
...
| `LEARNING`           | `STRUCTURE`, `ABSTRACTION`         | Varies by learner            |
...
| `ITERATIVE`   | High P, High T-identity | High J, High A                          |
| `ABSTRACTION` | High N, High T          | High S                                  |
| `AGENTIC`     | High T, High P          | High N (ungrounded), High J (premature) |
...
- **Strengths**: Award-winning structure (Singapore GPT-4 competition), explicit tone/audience targeting, highly
  adaptable
- **Weaknesses**: May feel over-structured for simple tasks
- **Complexity**: Medium
- **Time Investment**: Medium
- **Source**: Developed by Sheila Teo
...
- **Strengths**: Streamlined, clear task specification, explicit I/O requirements
- **Weaknesses**: Less suitable for creative or ambiguous tasks
- **Complexity**: Low
- **Time Investment**: Low

#### CRAFT/CRAFTED Framework
...
- **Strengths**: Comprehensive constraint-setting, output templating, optional iteration
- **Weaknesses**: More complex setup than simpler frameworks
...
- **Code**: `SELF_REFINE`
- **Components**: Initial Generation → Self-Feedback → Refinement → Iteration
- **Best For**: Writing improvement, code refinement, quality-sensitive outputs
- **Strengths**: Autonomous quality improvement, no human intervention needed per cycle
- **Weaknesses**: Requires clear quality criteria, may over-iterate
- **Complexity**: High
...
- **Code**: `REFLEXION`
- **Components**: Action → Evaluation → Reflection → Memory → Retry
- **Best For**: Complex problem-solving, code generation, multi-step reasoning tasks
- **Strengths**: Learns from mistakes, maintains memory across attempts, verbal reinforcement
- **Weaknesses**: Complex to implement, requires clear success signals
- **Complexity**: High
- **Time Investment**: High
...
- **Code**: `STEP_BACK`
- **Components**: Abstraction → Grounding → Reasoning
- **Best For**: STEM problems, knowledge-intensive questions, multi-hop reasoning
- **Strengths**: 36% improvement over Chain of Thought (DeepMind), principle-based thinking
- **Weaknesses**: May feel unnecessary for simple questions
- **Complexity**: Medium
- **Time Investment**: Medium
...
- **Code**: `REACT`
- **Components**: Thought → Action → Observation (interleaved loop)
- **Best For**: Tool-using tasks, information retrieval, multi-step decision-making, agentic workflows
- **Strengths**: Enables external tool interaction, grounded reasoning, adaptable
- **Weaknesses**: Requires tool availability, complex to implement
- **Complexity**: High
- **Time Investment**: High
...
- **Code**: `SKELETON_OF_THOUGHT`
- **Components**: Skeleton Generation → Parallel Expansion → Assembly
- **Best For**: Structured explanations, listicles, consultancy responses
- **Strengths**: Reduces latency 2x+, improves quality in 60% of cases (Microsoft Research)
- **Weaknesses**: NOT suitable for sequential reasoning (math, coding)
- **Complexity**: Medium
...
- **Code**: `META_PROMPTING`
- **Components**: Task Analysis → Framework Selection → Prompt Generation → Self-Critique → Refinement
- **Best For**: Prompt optimisation, complex task setup, bootstrapping better prompts
- **Strengths**: Leverages AI for prompt engineering, comprehensive approach
- **Weaknesses**: Requires good initial task description, meta-level complexity
- **Complexity**: High
- **Time Investment**: Medium-High
- **Source**: Stanford NLP 2023
...
| Step-Back         | Primary     | Abstraction helps identify decision principles  |
...
| ReAct             | Secondary   | When decision requires information gathering    |
...
| CO-STAR          | Primary     | Precise tone control for strategic communications |
...
| Step-Back         | Primary     | Principle-based analysis          |
...
| ReAct             | Secondary   | Analysis requiring tool use       |
...
| CO-STAR             | Primary     | Award-winning tone/audience control        |
| CRAFT               | Primary     | Comprehensive constraint-setting           |
...
| Skeleton-of-Thought | Secondary   | Structured parallel content development    |
| Self-Refine         | Secondary   | Quality-critical content                   |
...
| ICIO        | Primary     | Clear input/output specification           |
...
| Self-Refine | Secondary   | Code refinement and quality                |
| Reflexion   | Secondary   | Complex code generation                    |
...
| Skeleton-of-Thought                 | Primary     | Structured idea generation       |
...
| ReAct             | Primary     | Tool-using problem solving                 |
| Step-Back         | Primary     | Principle-based problem solving            |
...
| Reflexion         | Secondary   | Learning from failed attempts              |
...
| Step-Back           | Primary     | Principle-first learning         |
| Socratic Method     | Secondary   | Learning through questioning     |
| Few-Shot            | Secondary   | Learning by example              |
| Skeleton-of-Thought | Secondary   | Structured explanations          |
...
| CO-STAR                    | Primary     | Tone and audience control                      |
...
| Self-Refine      | Primary     | Iterative feedback and improvement               |
...
4. **Filter by suitability**: Prefer "Primary" frameworks unless task has special characteristics
...
7. **Consider user's time constraints** if mentioned
8. **Select ONE primary framework** for prompt construction
9. **Note secondary frameworks** that could be mentioned as alternatives
...
- **Iterative**: Tasks where refinement cycles improve output significantly → Add Self-Refine or Reflexion as secondary
  option
...
The `cognitive_requirements` field is essential for Task-Trait Alignment analysis in the personality calibration stage.
It determines which user traits should be amplified, counterbalanced, or treated neutrally.
