...
**Issue:** Fallback assertion `expect(hasFrameworkIndicator || page.url()).toBeTruthy()` always passes because
`page.url()` returns a string (always truthy).
...
**Issue:** Tests creating new pages with `context.newPage()` were writing to main 'personality' database instead of '
bettrprompt_e2e' test database.

**Root Cause:** New pages weren't setting up X-Test-Auth header routing before navigation, causing requests to use wrong
database.
